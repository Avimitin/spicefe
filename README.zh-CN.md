...
`spicefe` 是一个可在全球静态托管、用于 spice2x 副屏视频流、机台按键及旧版
beatmania IIDX 机台米字屏的局域网客户端。在手机、平板或其他现代浏览器中打开
页面，选择已保存的游戏 PC，浏览器就会直接连接 spice2x，提供视频与触控输入、
顶栏街机控制，或显示 9 字符米字屏内容。
...
- 提供仅使用 API 的街机数字键盘弹出面板，包含 0–9、独立 `00` 键、
  开始与帮助；测试与服务放在所有模式都可用的独立防误触面板中
- 新服务器使用分步向导创建：先验证 API 地址，再依次选择游戏图标、连接方式、
  可选的串流画质设置并命名
...
- 在每个服务器旁分别显示主机、控制 API 与视频服务器状态及各自的失败原因；
  服务器列表可见时每分钟刷新一次
- 支持鼠标、单点触控和多点触控输入
- 视频始终保持宽高比自适应显示，并正确映射触控坐标
- 在紧凑的串流顶部栏显示实时分辨率、实测帧率与主机内存占用
...
- 串流页面可见时自动保持屏幕常亮：优先使用浏览器屏幕唤醒锁，HTTP 页面使用
  本地无声视频作为兼容方案；断开连接、串流出错或切到后台时释放，效果受浏览器
  节能策略限制
- 自适应手机、平板和桌面设备的界面
...
| --- | --- | --- |
| 触控、游戏信息、数字键盘、卡片导入/插卡及 IIDX 米字屏 | `ws://PC:1338` | 卡片导入要求设置 spice2x 密码；使用旧式 RC4 |
...
分配画面捕获前响应，因此不会启动编码器，也不会与视频观看者争用画面。米字屏及
数字键盘配置绝不会访问视频端点；米字屏会在 API 检查成功后发送只读的
`iidx/ticker_get` 请求。
...
画面模式、IIDX 米字屏模式及仅使用 API 的控制模式。本地上传的自定义图标不会随链接导出；接收设备会
改用默认 spice2x 图标。
...
顶栏启动按钮；副屏视频、米字屏和仅使用 API 的控制会话中都可使用。当前游戏未提供的
按键会保持禁用。每个持续按下的机台按键都会在指针或键盘操作结束、页面失去焦点或断开连接时明确释放。
...
视频、米字屏或仅使用 API 的控制会话连接后，点击顶部栏中的卡片图标，选择玩家 1 或玩家 2，再选择卡片。
spicefe 会通过当前控制 API 连接发送原生的
`card.insert(reader, card_id)` 请求，并自动关闭菜单。

## 游戏 PC 设置

请安装[最新版 spice2x](https://github.com/spice2x/spice2x.github.io/releases/latest)。
最低支持版本为
[`spice2x-26-09-01`](https://github.com/spice2x/spice2x.github.io/releases/tag/26-09-01)，
其中包含所需的控制 API 及浏览器支持。连接时 spicefe 会读取 `info.launcher`；
若版本更旧或无法识别，会显示带最新版下载链接的警告，但不会阻止连接。
使用副屏视频时，以类似下方的参数启动游戏：

```text
spice64.exe ... -api 1337 -apipass choose-a-lan-password -apistream
```

旧版 IIDX 米字屏和仅使用 API 的控制只需要 API，不需要视频服务器：
