...
- 提供浏览器本地 e-amusement 卡片库，支持原生格式卡号生成、从串流工具栏向
  P1/P2 插卡，以及自定义卡片外观
...
| --- | --- | --- |
| 触控、游戏信息及插卡 | `ws://PC:1338` | 可选 spice2x 密码；使用旧式 RC4 |
...
字符；如果曾使用 spice 的卡片生成功能，也可从 spice 目录中的 `card0.txt` 复制
现有 ID。手动输入的 ID 必须为 16 位十六进制字符。
...
视频开始播放后，点击顶部栏中的卡片图标，选择玩家 1 或玩家 2，再选择卡片。
