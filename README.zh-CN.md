...
- 提供仅使用 API 的全屏机台数字键盘，包含 0–9、开始、帮助、测试及服务键，
  无需开启副屏视频流
...
画面模式、IIDX 米字屏模式及全屏数字键盘模式。本地上传的自定义图标不会随链接导出；接收设备会
改用默认 spice2x 图标。
...
在连接配置中启用**全屏数字键盘**，即可在不打开副屏视频流的情况下操作机台按键。
数字键盘与米字屏模式互斥；两者都会隐藏视频画质设置，并且只连接 spice2x 控制 API。

主键区通过 `keypads.write()` 向玩家 1 发送数字 0–9。开始、帮助、测试及服务键会
匹配 `buttons.read()` 返回的按键名称；当前游戏未提供的按键会保持禁用。每个机台
按键都会在指针或键盘操作结束、页面失去焦点或断开连接时明确释放。
...
视频、米字屏或数字键盘会话连接后，点击顶部栏中的卡片图标，选择玩家 1 或玩家 2，再选择卡片。
...
旧版 IIDX 米字屏和全屏数字键盘只需要 API，不需要视频服务器：
